...
* Can you think of alternative ways to cluster or bin your data that might change the observed number of species?
    * Split the filamentous (Twizlers) into individual strands, so we 
    * Bundling the candy by shape only and not colour, reducing the number of species 
* How might different sequencing technologies influence observed diversity in a sample?
* OTU criteria
    * Consistent sampling methods in a certain environment - should change by environment
    * Clearly defined species definition (97% sequence, 100% ASV)
        * Can have strain; one below species
        * Everything above species should be conserved 
    * Same gene region
    * Same primers
    * Use curated datbases - 
    * Consistent DNA extraction methods -> some are biased 